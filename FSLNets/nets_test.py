...
from nets_netmats import nets_netmats
from nets_groupmean import nets_groupmean
from nets_tsclean import nets_tsclean
import numpy as np
...
ts_dir='/vol/bitbucket/ta2812/Data/random/100307/L/fmri_dr/' # dual regression output directory, containing all subjects' timeseries

# load timeseries data from the dual regression output directory
ts = TimeSeriesData.nets_load(ts_dir,3,1);
...
#To view the nodepics you must have already run the following (outside MATLAB), to create summary pictures of the maps in the NIFTI file:
...
savedir = '/vol/bitbucket/ta2812/Data/netmat_test/python/'
netmats0=  nets_netmats(ts,0,'cov');        # covariance (with variances on diagonal)
netmats1z=  nets_netmats(ts,1,'corr');       # full correlation (normalised covariances)
netmats2z=  nets_netmats(ts,1,'icov');       # partial correlation
netmats5z=  nets_netmats(ts,1,'ridgep');     # Ridge Regression partial, with rho=0.1
netmats1=  nets_netmats(ts,0,'corr');       # full correlation (normalised covariances)
netmats2=  nets_netmats(ts,0,'icov');       # partial correlation
netmats5=  nets_netmats(ts,0,'ridgep', 0.01);     # Ridge Regression partial, with rho=0.1

np.savetxt(savedir + '/netmats-corr-nozstat.txt', netmats1)
np.savetxt(savedir + '/netmats-partialcorr-nozstat.txt', netmats2)
np.savetxt(savedir + '/netmats-ridgereg-nozstat.txt', netmats5)
np.savetxt(savedir + '/netmats-corr.txt', netmats1z)
np.savetxt(savedir + '/netmats-partialcorr.txt', netmats2z)
np.savetxt(savedir + '/netmats-ridgereg.txt', netmats5z)
np.savetxt(savedir + '/netmats-cov.txt', netmats0)
...
[Znet1,Mnet1]=nets_groupmean(netmats0,1);   # test whichever netmat you're interested in; returns Z values from one-group t-test and group-mean netmat
